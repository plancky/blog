...
>"KJ was 10mins late, Astra 30, Skye 15, Reyna 20, and yeah how could I forget Sage who was 1 hour late." you said to yourself.  

You decide that this time you aren't going to wait any longer than 25mins this time around, so intuitively, you calculate that the chance that you will leave alone just from your prior experiences is 
...
We call a single performance of an experiment a **trial** and each possible result an **outcome**.

The **sample space** $S$ of the experiment is then the set of all possible outcomes of an individual trial. 

Most often, one is not interested in individual outcomes but in whether an outcome belongs to a given subset A (say) of the sample space S; Any **event** $A$ is a subset of the sample space $S$. $A \subseteq S$
...
2.  If A and B are two events in S then,$$ P(A\cup B) = P(A) + P(B) - P(A \cap B)$$ if $A$ and $B$ are **mutually exclusive**, $A \cap B = \phi \implies P(A\cup B) = P(A) + P(B)$. When the probabilities are combined to calculate the probability for the union of the $n$ general events, the result, which may be proved by induction upon $n$, is $$P\left(\bigcup_i^nA_i \right) = \sum_i^nP(A_i) - \sum_{i\neq j}P(A_i \cap A_j) + \sum_{i\neq j\neq k} P(A_i\cap A_j\cap A_k) -\dots+(-1)^{n+1}P\left(\bigcap_i A_i\right)$$
